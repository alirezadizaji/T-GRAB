...
class SpatialLongRangeCauseEffect(GraphGenerator):
    ...
    def __init__(self, args) -> None:
        ...
        self.dataset_name = self.dataset_name + f"/SLRCE-V2-star-{args.num_samples}ns-{args.num_active_branches}anb-{args.num_branches}nb-{args.val_ratio}vr-{args.test_ratio}tr"
        self.args = args
    ...
    def get_parser():
        ...
        parser.add_argument("--num-active-branches", type=int, required=True)
    ...
    def _create_star_with_paths(self, num_active_branches, num_branches, path_length):
        G = nx.Graph()
        ...
        for i in range(num_active_branches):
        ...
        for i in range(num_active_branches, num_branches):
            for j in range(path_length):
                G.add_node(current_node)
                current_node += 1
    ...
    def get_links(self, args_dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        G_star, end_nodes = self._create_star_with_paths(num_active_branches=self.args.num_active_branches, 
                                                        num_branches=self.args.num_branches, 
                                                        path_length=self.branch_len)
