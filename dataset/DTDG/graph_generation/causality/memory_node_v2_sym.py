"""
Symmetric version of "Version2 of memory node": odd days represent the edges between memory node and source nodes at previous time step; Even days represents a random generated edge between non-memory nodes.
"""
...
def main():
    ...
    num_neg_edge = None
    p_edge = 0.5
    name = f"mem-node-single-edge-{num_weeks-val_weeks-test_weeks}w-n{num_nodes}-sym"
    seed = 12345
    save_dir = "./TSA/data"


    test_T = T - test_weeks * num_days
    val_T = test_T - val_weeks * num_days
    ...
    for i in range(T):
        if i % 2 == 0:
            ...
            src1 = np.concatenate([A[:, 0], A[:, 1]])
            dst1 = np.concatenate([A[:, 1], A[:, 0]])
            num_edges = src1.size
            src = np.concatenate([src, src1])
            dst = np.concatenate([dst, dst1])
        else:
            # Memory node's edges.  
            src2 = np.full_like(np.unique(src1), fill_value=mem_node_id)
            dst2 = np.unique(src1)
            sym_src2 = np.concatenate([src2, dst2])
            sym_dst2 = np.concatenate([dst2, src2])
            num_edges = sym_src2.size
            src = np.concatenate([src, sym_src2])
            dst = np.concatenate([dst, sym_dst2])
