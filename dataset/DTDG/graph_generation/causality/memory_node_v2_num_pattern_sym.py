...
def main():
    ...
    num_neg_edge = None
    p_edge = 0.5
    name = f"mem-node-{num_patterns}-pattern-{num_weeks-val_weeks-test_weeks}w-n{num_nodes}-sym"
    seed = 12345
    save_dir = "./TSA/data"
    ...
    for i in range(1, num_cycles):
        max_t = t.max()
        t = np.concatenate([t, one_t + max_t + 1])
    
    edge_feat = np.tile(edge_feat, num_cycles)
    src = np.tile(src, num_cycles)
    dst = np.tile(dst, num_cycles)

    remained = t.size - src.size
