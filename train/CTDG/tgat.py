...
class TGATTrainer(CTDGTrainer):
    ...
    def set_model_args(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument('--num-neighbors', type=int, help='TGAT: number of neighbors to sample for each node', default=20)
    ...
    def get_model(self) -> Dict[str, Module]:
        models = super(TGATTrainer, self).get_model()
        ...
        backbone = TGAT(node_raw_features=full_dataset._node_feat.numpy(), 
                        edge_raw_features=full_dataset.edge_feat.numpy(), 
    ...
    def _get_run_save_dir(self) -> str:
        ctdgtrainer_run_save_dir = super(TGATTrainer, self)._get_run_save_dir()
