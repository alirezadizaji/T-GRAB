...
class LinkPredTrainer(CTDGTrainer):
    def __init__(self):
    ...
    def _set_running_args(parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument('--train-snapshot-based', action='store_true', help='If given, then apply "snapshot-based" training, otherwise normally apply "batch of links" training.')
        return parser
    ...
    def _get_run_save_dir(self) -> str:
        ...
        return os.path.join(
            ...
            "linkpred",
            self.args.data,
            f"snapshot_training={self.args.train_snapshot_based},batch_size={self.args.train_batch_size}")
    
    def create_data_loaders(self) -> Tuple[torchDataLoader, torchDataLoader, torchDataLoader]:
        if self.args.train_snapshot_based:
            to_dense = self.args.eval_mode
            return_edge_info = not self.args.eval_mode
            ...
            train_dataset = LinkPredNodeFeatureStaticDataset(
                os.path.join(self.args.root_load_save_dir, self.args.data_loc), 
                            ...
                            to_dense=to_dense,
                            return_edge_info=return_edge_info)
            
            # This collate function assures each sparse tensor has the same length. 
            # To access the real information, use the `mask` tensor to pass the real length.
            def collate_fn_sparse_tensors(batch):
                shapes = np.array([item[0][0].shape for item in batch])
                max_len = shapes.max()
                
                new_batch = []
                for item in batch:
                    (curr_src, curr_dst), node_feat, curr_t, curr_edge_feat, curr_edge_id = item
                    len = curr_src.shape[0]
                    remain_len = max_len - len
                    new_curr_src = torch.cat([curr_src, torch.zeros((remain_len, *curr_src.shape[1:]), dtype=curr_src.dtype)])
                    new_curr_dst = torch.cat([curr_dst, torch.zeros((remain_len, *curr_dst.shape[1:]), dtype=curr_dst.dtype)])

                    new_curr_edge_feat = torch.cat([curr_edge_feat, torch.zeros((remain_len, *curr_edge_feat.shape[1:]), dtype=curr_edge_feat.dtype)])
                    new_curr_edge_id = torch.cat([curr_edge_id, torch.zeros((remain_len, *curr_edge_id.shape[1:]), dtype=curr_edge_id.dtype)])

                    mask = torch.zeros(max_len, dtype=bool)
                    mask[:len] = True
                    new_item = ((new_curr_src, new_curr_dst), node_feat, curr_t, new_curr_edge_feat, new_curr_edge_id, mask)
                    new_batch.append(new_item)
                
                return default_collate(new_batch)
            
        else:
            train_dataset = ContinuousTimeLinkPredNodeFeatureStaticDataset(
                                os.path.join(self.args.root_load_save_dir, self.args.data_loc), 
                                self.args.data, 
                                split="train", 
                                node_feat=self.args.node_feat, 
        ...
        if self.args.train_snapshot_based:
            if self.args.eval_mode:
                ctdg_collate_fn=default_collate
            else:
                ctdg_collate_fn=collate_fn_sparse_tensors
            # During `snapshot-based` training, It's important to keep the order of snapshots.
            # No shuffling should happen.
            shuffle = False
        else:
            ctdg_collate_fn=default_collate
            shuffle = True

        train_loader = torchDataLoader(train_dataset, batch_size=self.args.train_batch_size, collate_fn=ctdg_collate_fn, shuffle=shuffle)
    ...
    def _train_for_one_epoch_batch_link(self):
    ...
    def train_for_one_epoch(self):
        if self.args.train_snapshot_based:
            assert isinstance(self.train_loader.dataset, LinkPredNodeFeatureStaticDataset)
            # assert self.args.train_batch_size == 1, f"Snapshot training can only be done with batch size of one; Got the batch size of {self.args.train_batch_size} instead."
            return self._train_for_one_epoch_snapshot_based()
        else:
            assert isinstance(self.train_loader.dataset, ContinuousTimeLinkPredNodeFeatureStaticDataset)
            return self._train_for_one_epoch_batch_link()
    ...
    def eval_for_one_epoch(self, split_mode: str):
        ...
        if split_mode == 'train':
            ...
            assert isinstance(eval_loader.dataset, LinkPredNodeFeatureStaticDataset), "CTDG Train loader being used during evaluation should be an instance of `LinkPredNodeFeatureStaticDataset`."
        elif split_mode == 'val':
