...
class EdgeBankLongRangeTrainer(LinkPredEdgeBankTrainer):
    """ This class is only useful to train long-range-based datasets. 
    For more info about long-range datasets, please checkout the documentation of `LongRangeGenerator` class.
    """
    def __init__(self):
        super(EdgeBankLongRangeTrainer, self).__init__()
    ...
    def update_metrics(self, 
        ...
        whole_graph_precision = precision_score(whole_graph_ground_truth.cpu().numpy(), whole_graph_pred_bin)
        whole_graph_recall = recall_score(whole_graph_ground_truth.cpu().numpy(), whole_graph_pred_bin)
        whole_graph_f1 = f1_score(whole_graph_ground_truth.cpu().numpy(), whole_graph_pred_bin)
        whole_graph_acc = accuracy_score(whole_graph_ground_truth.cpu().numpy(), whole_graph_pred_bin)
