...
class LinkPredPreviousTrainer(LinkPredTrainer):
    def __init__(self):
    ...
    def get_optimizer(self) -> torch.optim.Optimizer:
        return NotImplementedError("Previous model does not support getting optimizer.")
    
    def get_criterion(self) -> torch.nn.Module:
        return NotImplementedError("Previous model does not support getting criterion.")
    ...
    def train_for_one_epoch(self, split_mode: str):
        raise NotImplementedError("Previous model does not support training.")

    def early_stopping_checker(self, early_stopper) -> bool:
        raise NotImplementedError("Previous model does not support early stopping.")
    
    def forward_backbone(self, snapshot: torch.Tensor, snapshot_feat: torch.Tensor, node_feat: torch.Tensor) -> torch.Tensor:
        raise NotImplementedError("Previous model does not support forward backbone.")
    ...
    def get_dataset_regex_pattern(self) -> str:
        ...
        raise NotImplementedError("Previous model does not support getting dataset regex pattern.")

    def _get_run_save_dir(self) -> str:
        linkpredtrainer_run_save_dir = super(LinkPredPreviousTrainer, self)._get_run_save_dir()
    ...
    def list_of_metrics_names(self) -> List[str]:
        raise NotImplementedError("Previous model does not support getting list of metrics names.")